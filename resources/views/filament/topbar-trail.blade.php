{{--
    The top bar's company switcher and the trail of the page (top-bar trail
    spec §2, as redesigned in breadcrumb2).
    ...
    On desktop the switcher takes the brand's place — topbar-styles hides the
    logo and gives the switcher the sidebar's width — and the trail sits in
    data-topbar-column, which topbar-styles lays over the page's content
    column, so the first crumb starts where the heading below it starts.
--}}
@php
                    ...
                    @foreach ($trail as $crumb)
                        <li class="app-trail-item">
                            {{-- Between crumbs only: the switcher is not part of the
                                 trail, so nothing precedes the first crumb. --}}
                            @unless ($loop->first)
